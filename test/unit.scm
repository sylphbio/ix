...
(test-group "ix"
  ...
  (test "build all primitive types (proto)" prims-oj (ix:build 'ix:test :sym 'a :oid oid :str "ok" :int -4 :nat 3 :sci 4.5 :t #t :f #f))
  (test-assert "all primitive types well-typed (proto)" (ix:well-typed? (from-just (ix:build 'ix:test :sym 'a :oid oid :str "ok" :int -4 :nat 3 :sci 4.5 :t #t :f #f))))
  (define prims-s-un (<> "(ix:* :sym a :oid \"" oid "\" :str \"ok\" :int -4 :nat 3 :sci 4.5 :t #t :f #f)"))
  (define prims-oj-un (parse:ix prims-s-un))
  (test "build all primitive types (generic)" prims-oj-un (ix:build 'ix:* :sym `(symbol a) :oid `(oid ,oid) :str `(string "ok") :int `(integer -4) :nat `(natural 3) :sci `(scientific 4.5) :t `(boolean #t) :f `(boolean #f)))
  (test-assert "all primitive types well-typed (generic)" (ix:well-typed? (from-just (ix:build 'ix:* :sym `(symbol a) :oid `(oid ,oid) :str `(string "ok") :int `(integer -4) :nat `(natural 3) :sci `(scientific 4.5) :t `(boolean #t) :f `(boolean #f)))))
