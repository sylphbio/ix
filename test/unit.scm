...
(test-group "ix"
  ...
  (define oid (uuid))
  (define all-prims `(ix:test :sym symbol :oid oid :str string :int integer :nat natural :sci scientific :t boolean :f boolean))
  (ix:init (lambda (_) `(Just ,all-prims)))
  (define prims-s (<> "(ix:test :sym a :oid \"" oid "\" :str \"ok\" :int -4 :nat 3 :sci 4.5 :t #t :f #f)"))
  (define prims-oj (parse:ix prims-s))
  (test "build all primitive types (checked)" prims-oj (ix:build 'ix:test :sym 'a :oid oid :str "ok" :int -4 :nat 3 :sci 4.5 :t #t :f #f))
  (test-assert "all primitive types well-typed (checked)" (ix:well-typed? (from-just (ix:build 'ix:test :sym 'a :oid oid :str "ok" :int -4 :nat 3 :sci 4.5 :t #t :f #f))))
  (define prims-s-un (<> "(ix:* :sym a :oid \"" oid "\" :str \"ok\" :int -4 :nat 3 :sci 4.5 :t #t :f #f)"))
  (define prims-oj-un (parse:ix prims-s-un))
  (test "build all primitive types (unchecked)" prims-oj-un (ix:build 'ix:* :sym `(symbol a) :oid `(oid ,oid) :str `(string "ok") :int `(integer -4) :nat `(natural 3) :sci `(scientific 4.5) :t `(boolean #t) :f `(boolean #f)))
  (test-assert "all primitive types well-typed (unchecked)" (ix:well-typed? (from-just (ix:build 'ix:* :sym `(symbol a) :oid `(oid ,oid) :str `(string "ok") :int `(integer -4) :nat `(natural 3) :sci `(scientific 4.5) :t `(boolean #t) :f `(boolean #f)))))
